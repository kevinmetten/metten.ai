...
internal interface FloatPcmCapture {
    fun start(consumer: (FloatArray) -> Unit): Result<Unit>
    fun stop()
    fun release()
}
...
internal class SoniqoAudioRecordCapture(
    ...
    override fun start(consumer: (FloatArray) -> Unit): Result<Unit> = runCatching {
        if (!running.compareAndSet(false, true)) return@runCatching
        ...
        worker.execute {
            ...
            while (running.get() && record === next) {
                val count = next.read(frame, 0, frame.size, AudioRecord.READ_BLOCKING)
